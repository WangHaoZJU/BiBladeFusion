...
当前完成的是库级协调、资产事务和失败关闭状态机，尚未提供机器人运动CLI。默认配置中
`stop_and_capture.enabled=false`、`occupancy.enabled=false`，短段关节上限也是`null`。
更重要的是，生产碰撞检查器还没有连续网格扫掠证明和连续机器人—占用体素扫掠证明，
因此真实运动仍然在预检阶段保持`MOTION_BLOCKED`。本页不构成真机放行声明。
...
离散关节采样不能冒充连续证明。因此当前`prepare_next_segment()`会返回阻断预检并进入
`MOTION_BLOCKED`，正常情况下不能抵达`WAITING_APPROVAL`，更不能调用机器人驱动。

## 8. 执行后stop与有界间隔停稳采样证据

未来两项连续扫掠证明完成后，每个批准段仍必须按以下顺序收尾：
