...
本次没有增加“自动连通域失败后复用上一视角 seed”的危险降级逻辑。第一帧 formal capture 使用
用户绑定的 hard ROI；第二、第三个人工视角及后续粗扫视角通常使用自动深度连通域，再经过 XYZ
包络。

自动连通域如果不明确、选不到内部目标、触及有效深度边界或保留点不足，会阻塞。不得为了继续
而自动沿用另一视角的二维 polygon，因为不同相机位姿下像素坐标不具有相同物理意义。
