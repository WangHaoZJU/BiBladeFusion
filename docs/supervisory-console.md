...
活动实验在规划和预检事件发生时，还会以原子替换方式更新
`live_timeline/live_planning.json`。这个轻量侧车不重复写点云或网格，也不重新执行IK、碰撞检测
或RRT。`--follow`模式只在会话ID和时间均与最新不可变快照一致时采用它，并在“候选视点与规划
预检”表中显示：科学排序/得分、当前候选、IK、端点碰撞、直线路径、RRT、已知耗时、阻断原因
和最终路径类型。没有独立计时证据的字段保持`UNKNOWN/PENDING`，不能从总耗时反推。

安全场景中同时绘制当前相机和候选相机视锥：当前为黄色、尚未预检的候选为灰色、正在预检的
候选为蓝色、最终选中候选为绿色。侧车及窗口仍然完全只读；写入或读取失败只会停用观察者，
不会中断感知、科学状态机或真机运动。当前实现仍是PySide桌面窗口，经SSH X11转发可能较卡。
后续浏览器化只允许复用HoloRobot的只读HTTP/SSE、时间线和Three.js显示子集，不能复制任何
控制路由。
